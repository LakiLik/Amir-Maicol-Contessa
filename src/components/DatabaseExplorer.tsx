    ...
    <div className="space-y-6 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b border-[var(--fg-color)] pb-6">
        ...
        <div className="flex items-center space-x-2 flex-wrap gap-y-2">
           <button onClick={fetchData} disabled={loading} className="text-xs border border-[var(--fg-color)] bg-[var(--card-bg)] px-4 py-3 font-bold uppercase tracking-widest hover:bg-[var(--fg-color)] hover:text-[var(--bg-color)] transition-colors shadow-[2px_2px_0px_0px_var(--fg-color)] active:shadow-none active:translate-y-[2px] active:translate-x-[2px] disabled:opacity-50 flex items-center gap-2">
             <RefreshCw size={14} className={loading ? 'animate-spin' : ''} /> Aggiorna
           </button>
        </div>
      </div>

      <div className="flex space-x-4 border-b border-[var(--fg-color)] pb-2">
         <button onClick={() => setActiveTab('explorer')} className={`text-[11px] font-bold uppercase tracking-widest px-2 py-1 ${activeTab === 'explorer' ? 'border-b-2 border-[var(--fg-color)]' : 'opacity-60'}`}>Data Explorer</button>
         <button onClick={() => setActiveTab('backup')} className={`text-[11px] font-bold uppercase tracking-widest px-2 py-1 ${activeTab === 'backup' ? 'border-b-2 border-[var(--fg-color)]' : 'opacity-60'}`}>Backup & Restore</button>
         <button onClick={() => setActiveTab('audit')} className={`text-[11px] font-bold uppercase tracking-widest px-2 py-1 ${activeTab === 'audit' ? 'border-b-2 border-[var(--fg-color)]' : 'opacity-60'}`}>Audit Logs</button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
         ...
         <div className="md:col-span-1 space-y-2">
            <h2 className="text-xs font-bold uppercase tracking-widest border-b border-[var(--fg-color)] pb-2 mb-4">Collezioni</h2>
            {COLLECTIONS.map(col => (
               <button
                  key={col}
                  onClick={() => setActiveCollection(col)}
                  className={`w-full text-left px-4 py-3 text-[10px] font-bold uppercase tracking-widest border border-[var(--fg-color)] transition-all ${activeCollection === col ? 'bg-[var(--fg-color)] text-[var(--bg-color)] shadow-[4px_4px_0px_0px_rgba(0,0,0,0.2)]' : 'bg-[var(--card-bg)] hover:bg-[var(--bg-color)] shadow-[2px_2px_0px_0px_var(--fg-color)]'}`}
               >
            ...
            {!loading && (
               <div className="mt-8 p-4 border border-[var(--fg-color)] bg-[var(--card-bg)] shadow-[4px_4px_0px_0px_var(--fg-color)]">
               ...
               <div className="flex gap-2 w-full md:w-auto">
                  <div className="relative flex-1 md:w-64">
                     <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <Search size={14} className="text-[var(--fg-color)] opacity-50" />
                     </div>
                     <input
                        type="text"
                        ...
                        className="w-full border border-[var(--fg-color)] bg-[var(--card-bg)] py-2 pl-9 pr-3 text-xs font-mono focus:outline-none focus:ring-1 focus:ring-[var(--fg-color)]"
                     />
                  </div>
                  <button onClick={exportCollectionJSON} disabled={loading || filteredData.length === 0} className="text-[10px] font-bold uppercase tracking-widest border border-[var(--fg-color)] bg-[var(--card-bg)] px-3 py-2 hover:bg-[var(--fg-color)] hover:text-[var(--bg-color)] transition-colors flex items-center gap-2 disabled:opacity-50 whitespace-nowrap">
                     <FileJson size={12} /> Export
                  </button>
               </div>
            </div>

            <div className="border border-[var(--fg-color)] bg-[var(--fg-color)] shadow-[8px_8px_0px_0px_rgba(0,0,0,0.2)] p-2">
      ...
      {activeTab === 'backup' && (
        <div className="grid md:grid-cols-2 gap-8">
           <div className="border border-[var(--fg-color)] bg-[var(--card-bg)] p-8 shadow-[4px_4px_0px_0px_var(--fg-color)]">
              <div className="w-12 h-12 bg-[var(--fg-color)] text-[var(--bg-color)] flex items-center justify-center mb-6">
                <Download size={24} />
              </div>
              ...
              <button onClick={exportAllJSON} disabled={loading} className="w-full justify-center text-xs border border-[var(--fg-color)] bg-[var(--fg-color)] text-[var(--bg-color)] px-4 py-4 font-bold uppercase tracking-widest hover:bg-[var(--card-bg)] hover:text-[var(--fg-color)] transition-all shadow-[4px_4px_0px_0px_rgba(0,0,0,0.3)] hover:shadow-none hover:translate-y-[4px] hover:translate-x-[4px] disabled:opacity-50 flex items-center gap-2">
                 <FileJson size={16} /> Scarica Backup ({Object.keys(data).reduce((acc, c) => acc + (data[c]?.length || 0), 0)} records)
              </button>
           </div>
           
           <div className="border border-[var(--fg-color)] bg-[var(--card-bg)] p-8 shadow-[4px_4px_0px_0px_var(--fg-color)]">
              <div className="w-12 h-12 border-2 border-[var(--fg-color)] text-[var(--fg-color)] flex items-center justify-center mb-6">
                <UploadCloud size={24} />
              </div>
              ...
              <label className={`w-full justify-center text-xs border border-[var(--fg-color)] bg-[var(--card-bg)] text-[var(--fg-color)] px-4 py-4 font-bold uppercase tracking-widest hover:bg-[var(--fg-color)] hover:text-[var(--bg-color)] transition-all shadow-[4px_4px_0px_0px_var(--fg-color)] hover:shadow-none hover:translate-y-[4px] hover:translate-x-[4px] disabled:opacity-50 flex items-center gap-2 cursor-pointer ${isRestoring ? 'opacity-50 pointer-events-none' : ''}`}>
      ...
      {activeTab === 'audit' && (
         <div className="border border-[var(--fg-color)] bg-[var(--card-bg)] shadow-[4px_4px_0px_0px_var(--fg-color)] overflow-hidden">
            {auditLogs.length === 0 ? (
               ...
               <div className="divide-y divide-[var(--fg-color)]">
                 {auditLogs.map(log => (
                    <div key={log.id} className="p-4 flex flex-col md:flex-row md:items-center justify-between hover:bg-[var(--bg-color)] transition-colors">
                       <div>
                       ...
                       <div className="text-[9px] font-mono uppercase tracking-widest opacity-50 mt-2 md:mt-0 bg-[var(--card-bg)] border border-[var(--fg-color)] px-2 py-1">
